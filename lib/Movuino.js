...
class Movuino extends EventEmitter {
  ...
  /**
   * Attach serial port.
   * @returns {object} serialPort - the instantiated osc.SerialPort.
   */
  attachSerial() {
  ...
  /**
   * Detach serial port.
   * @returns {promise} detached - A Promise that resolves when the
   * serial port is detached.
   */
  detachSerial() {
   ...
   * @returns {promise<object>} - An object containing the same data formatted
   * as an OSC message, sent back by movuino to confirm the changes were aplied.
   */
  setWifi(config) {
    return serial.rpc(this.OSCSerialPort, "/wifi/set", [
   ...
   * @async
   * @returns {promise<object>} - An object containing 3 fields named
   ...
   */
  setRange(range) {
    return serial.rpc(this.OSCSerialPort, "/range/set", [
   ...
   * @async
   * @return {romise<object>} An object containing two fields named
   * accel and gyro, with the actual sensors' range values.
   */
  async getRange() {
    const [accel, gyro] = await serial.rpc(this.OSCSerialPort, "/range/get");
    return { accel, gyro };
  }
